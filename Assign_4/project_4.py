...
class Timecard_Win(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        #smallest number label
        self.small_num_label =  QtWidgets.QLabel(self)
        self.small_num_label.setFont(large_font)
        self.small_num_label.move(750,300)
        self.small_num_label.setText('Smallest Number:')
        ...
        #list for looking at smallest numbers
        self.num_list = [self.job_1_num, self.job_2_num, self.job_3_num, self.job_4_num, self.job_5_num,
                    self.job_1_phase, self.job_2_phase, self.job_3_phase, self.job_4_phase, self.job_5_phase] 
        ...
        #setting the style of the job # buttons
        ...
        #next smaller number button
        small_num_inc_btn = QtWidgets.QPushButton(self)
        small_num_inc_btn.setText('Next Smaller Number')
        small_num_inc_btn.setFont(large_bold_font)
        small_num_inc_btn.setStyleSheet("background-color: white; border: 2px solid black;")
        small_num_inc_btn.setGeometry(750,350,300,50)
        small_num_inc_btn.clicked.connect(self.small_num_inc)

    def small_num_inc(self):
        """Updates the display to show the next smaller number"""
        if len(self.num_list) >1:
            #want entire list except the first entry
            self.num_list = self.num_list[1:]
            self.small_num_label.setText(f'Smallest Number: {self.num_list[0]}')
            self.small_num_label.adjustSize()
    ...
    def smallest_num(self):
        """Displays the smallest number from inputs of job # and phase of the timesheet"""
        num_list = []
        self.num_list = [self.job_1_num, self.job_2_num, self.job_3_num, self.job_4_num, self.job_5_num,
                    self.job_1_phase, self.job_2_phase, self.job_3_phase, self.job_4_phase, self.job_5_phase] 
        #if input values can be floats, then they will be considered in a list of floats
        try:
            for ele in self.num_list:
                num_list.append(float(ele))
            self.num_list = sorted(num_list)
            self.small_num_label.setText(f'Smallest Number: {self.num_list[0]}')
            self.small_num_label.adjustSize()
        
        except ValueError:
            #if strings are just entered for the inputs, they will be ignored
            pass
